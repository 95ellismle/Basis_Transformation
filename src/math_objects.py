...
class MATH_OBJECTS(object):
    ...
    def __init__(self, txt, steps_taken, begin_eq, end_eq):
        self.txt = txt
        ...
        if self.objs:
            self.steps_taken += "Original Equation was:\n\n%s%s%s\n\n"%(begin_eq, self.latex(), end_eq)
            # Simplifying
    ...
    def _one_child_parent_sums(self):
        ...
        if self.latex() != orig_latex:
            self.steps_taken += "Split summations with multiple nesting into multiple parent sums and 1 child:\n\n%s%s%s\n\n"%(self.begin_eq, self.latex(), self.end_eq)
    ...
    def _split_sums(self):
            ...
            for i, (child,parent) in enumerate(self.struct[level]):
                if child.__name__ == "SUM":
                    ...
                    for objI in range(len(new_objs)):
                        NEW_OBJS[objI] = SUM("{_{%s} }"%",".join(inds), self.steps_taken, self.begin_eq, self.end_eq)
        ...
        if self.latex() != orig_latex:
            self.steps_taken += "Split summations with a + or - in (this is a code thing rather than a maths thing):\n\n%s%s%s\n\n"%(self.begin_eq, self.latex(), self.end_eq)
        self.struct, self.struct_names = self._create_struct(self.objs, self.obj_typ, parent="root", level=0, struct={}, levels={0:0}, names={})    
        self.paths  = self._create_paths(self.objs, parent_i=0, level=0, paths={})
        self._simplify_U()
   ...
   #self._remove_empty_sums()
    ...
    def _find_math_objects(self, txt):
        ...
        while (txt.find("{") != -1 and txt.find("}") != -1):
            ...
            obj = valid_math_objects[obj_type](obj_txt, self.steps_taken, self.begin_eq, self.end_eq)
    ...
    def latex(self):
        if not self.objs:
            return "No math objects found in ''%s'' "%self.txt
...
class SUM(MATH_OBJECTS):
    ...
    def _simplify_1_delta_with_indices(self, delta_inds, delta_I):
        change = False
        print("Before\n", self.latex(), "\n")
